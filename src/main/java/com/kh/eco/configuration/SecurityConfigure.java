	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		...
		return httpSecurity.formLogin(AbstractHttpConfigurer::disable)
						   ...
						   .authorizeHttpRequests(requests -> {
							   requests.requestMatchers(HttpMethod.POST, "/members/profile").permitAll(); 
							   requests.requestMatchers(HttpMethod.POST, "/auth/login", "/members/**", "/auth/refresh", "/auth/logout").permitAll(); // 누구나 허용할 기능
							   requests.requestMatchers(HttpMethod.PUT, "/members", "/members/**" , "/boards/**").authenticated(); // 수정, 인증 필요한 기능
							   ...
							   requests.requestMatchers(HttpMethod.GET, "/uploads/**", "/stats/**", "/api/**", "/api/boards/**", "/comments/**", "/uploads/**", "/feed/**", "/stats/today/**", "/api/boards/stats/**", "/members/**").permitAll(); // 게시글 전체조회 및 상세조회는 아무나
